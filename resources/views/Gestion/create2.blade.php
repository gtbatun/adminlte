            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    data.forEach(function(gestion) {
                        ...
                        gestionesHtml += '<span class="direct-chat-timestamp float-' + (isCurrentUser ? 'left' : 'right') + '">' + moment(gestion.created_at).fromNow() + '</span>';
                        gestionesHtml += '</div>';
                ...
                data.forEach(function(h_gestiones) {
                    var messageHtml =`
                        <div class="direct-chat-msg ${h_gestiones.user_id == {{ Auth::id() }} ? 'right' : ''}">
                            ...
                            <span class="direct-chat-name float-${h_gestiones.user_id == {{ Auth::id() }} ? 'right' : 'left'}">${h_gestiones.usuario.name}</span>                                
                            <span class="direct-chat-timestamp float-${h_gestiones.user_id == {{ Auth::id() }} ? 'left' : 'right'}">${moment(h_gestiones.created_at).fromNow()}</span>

                            </div> 
                            <img class="direct-chat-img" src="/storage/images/user/${h_gestiones.usuario.image}" alt="IMG">
                            <div class="direct-chat-text">
                                ${h_gestiones.coment}
                            </div>
                            <div class="direct-chat-name float-${h_gestiones.user_id == {{ Auth::id() }} ? 'right' : 'left'}">
